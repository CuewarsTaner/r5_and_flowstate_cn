...
void function _OnPlayerConnected(entity player)
{
	...
	if(FlowState_RandomGunsEverydie())
	    Message(player, "FLOWSTATE: FIESTA", "在控制台输入 commands 查看当前可用指令\n     KOOK频道:98171075  Q群:307689539 ", 10)
	else if (FlowState_Gungame())
	    Message(player, "FLOWSTATE: GUNGAME", "在控制台输入 commands 查看当前可用指令\n     KOOK频道:98171075  Q群:307689539 ", 10)
	else
	    Message(player, "FLOWSTATE: FFA/TDM", "在控制台输入 commands 查看当前可用指令\n     KOOK频道:98171075  Q群:307689539 ", 10)
...
void function __HighPingCheck(entity player)
{
	...
	if ( FlowState_KickHighPingPlayer() && (int(player.GetLatency()* 1000) - 40) > FlowState_MaxPingAllowed() )
	{
		...
		Message(player, "FLOWSTATE", "您当前的延迟: " + (int(player.GetLatency()* 1000) - 40) + " ms."
		, 5)
	}
}
        ...
        switch (player.GetPlayerGameStat( PGS_TITAN_KILLS ))
		{
			...
			case 5:
				...
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + "取得5杀!高手在此!", 4, "")
            break
            case 6:
			case 7:
				Inventory_SetPlayerEquipment(player, PURPLE_SHIELD, "armor")
			...
			case 15:
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + "取得15杀!杀敌如麻!", 5, "")
			break
			case 20:
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + "取得20杀!势不可挡!", 5, "")
			break
			case 25:
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + "取得25杀!天下无敌!", 5, "")
			break
			case 35:
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + "取得35杀!超越神啦!", 5, "")
			break
			case 50:
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + "取得50杀!这还是人吗!", 5, "")
            break
			default:
            break
        }
    ...
    if (died)
        ...
        switch (player.GetPlayerGameStat( PGS_TITAN_KILLS )) {
			...
			case 10:
				GiveFlowstateOvershield(player)
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + "取得10杀!护甲进化!", 5, "")
            break
			case 15:
				GiveFlowstateOvershield(player)
				foreach(sPlayer in GetPlayerArray())
			...
			case 20:
				GiveFlowstateOvershield(player)
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + "取得20杀!护甲进化!", 5, "")
            break
			case 25:
				GiveFlowstateOvershield(player)
				foreach(sPlayer in GetPlayerArray())
				Message(sPlayer,"猎杀播报", player.GetPlayerName() + "取得25杀!护甲进化!", 5, "")
            break
			default:
                break
        }
...
bool function ClientCommand_RebalanceTeams(entity player, array<string> args)
{
    if(IsAdmin(player)) {
        ...
        foreach (p in allplayers)
        {
            ...
            currentTeam += 1
            Message(p, "队伍调整", "我们现在有 " + numTeams + " 支小队", 4)
        }
		return true
	} else
		return false
